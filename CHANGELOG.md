...
> 本项目基于 [doocs/md](https://github.com/doocs/md) v2.1.0 二次开发

---

## [v0.1.0] - 2026-02-02 (md2pub 初始版本)
...
### [v2.1.0] - 2025-10-17 (原项目版本)

#### ✨ 新特性

- **AI 助手侧边栏 & 文生图**：新增独立的 AI 助手侧边栏，支持智能对话、文本生成与 AI 图像生成功能
- **PlantUML & Ruby 语法支持**：支持 PlantUML 图表渲染和 Ruby 文本标注（注音符号）
- **图片压缩 & 上传进度条**：上传图片时自动压缩并显示进度条
- **移动端适配增强**：进一步优化移动端显示与交互体验

#### 🏗️ 架构与部署

- **CodeMirror 升级至 v6**：编辑器核心组件全面升级
- **Vite 升级至 v7**：构建工具全面升级
- **pnpm Monorepo 重构**：项目重构为 pnpm monorepo 架构
- **Cloudflare Workers 部署**：支持 Cloudflare Workers 部署与自动化 CI/CD
